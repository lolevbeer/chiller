...
// deliberate write function this project has — writeSetpoint(), used by the
// opt-in setpoint-boost automation (lib/boost.js) and the dashboard's manual
// setpoint control (POST /api/setpoint in lib/routes.js), both gated on
// SETPOINT_WRITE=1.
...
/** Connect a fresh client to the chiller (unit 1). Callers destroy() it in finally.
 * @param {InstanceType<typeof ModbusRTU>} c */
async function connect(c) {
  c.setTimeout(3000);
  // connectTCP has no connect timeout of its own (setTimeout above only covers
  // responses) — race it so an unreachable chiller fails in 5 s, not the OS's
  // ~75 s. Promise.race marks the loser handled, so its rejection would
...
/**
 * Poll a readback until it lands within 0.05°F of wantF or attempts run out,
 * returning the final reading either way. The c.pCO refreshes the input-register
 * mirror on its own scan cycle, so reading immediately after FC6 races it and
 * can return the STALE setpoint (measured live 2026-07-18: the accepted write
 * showed the old value on instant readback, the new one seconds later). Without
 * this patience a real write reads as a mismatch — which lib/boost.js treats as
 * a wrong register and latches all writes off. delayMs injectable for tests.
 * @param {() => Promise<number>} readF current readback in °F
 * @param {number} wantF
 * @param {number} attempts
 * @param {number} delayMs
 * @returns {Promise<number>}
 */
async function verifySetpoint(readF, wantF, attempts = 6, delayMs = 500) {
  let readback = await readF();
  while (Math.abs(readback - wantF) > 0.05 && --attempts > 0) {
    await new Promise((r) => setTimeout(r, delayMs));
    readback = await readF();
  }
  return readback;
}
...
async function writeSetpoint(valueF) {
  ...
  try {
    ...
    await w;
    const readback = await verifySetpoint(
      async () => scale((await c.readInputRegisters(70, 1)).data[0]), valueF);
...
module.exports = { LABELS, scale, connect, read, writeSetpoint, verifySetpoint };
