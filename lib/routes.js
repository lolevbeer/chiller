 ...
 */
// The gate both write routes share: POST with a JSON body, else [status, reason].
...
// server never grants).
/** @param {import("http").IncomingMessage} req @returns {[number, string] | null} */
const notJsonPost = (req) => req.method !== "POST" ? [405, "POST only"]
  : String(req.headers["content-type"] || "").split(";")[0].trim().toLowerCase() !== "application/json"
    ? [400, "Content-Type must be application/json"] : null;
...
async function handleSetpoint(req, res, deps = { writeSetpoint }) {
  ...
  const bad = notJsonPost(req);
  if (bad) return reply(bad[0], { ok: false, error: bad[1] });
  ...
  if (!r.ok) {
    ...
    if (r.wrote && process.env.SLACK_WEBHOOK_URL) {
      (deps.post ?? require("./slack").post)({ text:
        `*Warning: Unverified manual setpoint write from the dashboard*\n` +
        `Sent ${targetF.toFixed(1)}°F but could not verify that it took effect` +
        `${typeof r.readback === "number" ? `; the setpoint reads back ${r.readback.toFixed(1)}°F` : ""}. Please verify the controller.` });
    }
  ...
  if (process.env.SLACK_WEBHOOK_URL) {
    (deps.post ?? require("./slack").post)({ text:
      `*Notice: Chiller setpoint changed to ${r.readback.toFixed(1)}°F from the dashboard*\n` +
      `The controller shuts the unit down when supply exceeds setpoint + ${TRIP_F}°F; ` +
      `the shutdown alarm will now trip at ${(r.readback + TRIP_F).toFixed(1)}°F.` });
  }
}

/**
 * POST /api/rearm-logger — the dashboard's "Re-arm logger" button. No body is
 * read; the JSON content type is required only as the CSRF gate (see
 * handleSetpoint). Answers {ok, message}: 200 when the keypad walk reached
 * RESTART LOGS, 409 when it refused or backed out (lib/pgd.js explains why in
 * `message`, and has already posted the outcome to Slack).
 * @param {import("http").IncomingMessage} req @param {import("http").ServerResponse} res
 * @param {{rearmLogger: typeof rearmLogger}} deps injectable for test.js
 */
async function handleRearm(req, res, deps = { rearmLogger }) {
  const reply = (/** @type {number} */ code, /** @type {object} */ obj) => {
    res.writeHead(code, { "Content-Type": "application/json" });
    res.end(JSON.stringify(obj));
  };
  const bad = notJsonPost(req);
  if (bad) return reply(bad[0], { ok: false, message: bad[1] }); // the page reads .message here
  const r = await deps.rearmLogger({ source: "the dashboard" });
  reply(r.ok ? 200 : 409, r);
}
...
async function handle(req, res) {
  ...
  if (url === "/api/rearm-logger") return handleRearm(req, res);
  if (url === "/api/all") {
...
module.exports = { handle, handleSetpoint, handleRearm, PAGE };
