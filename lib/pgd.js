 ...
 * @param {Pad} pad @param {typeof readAlarms} alarmsOf re-checked before the press
 * @returns {Promise<{error?: string, answer?: string}>} */
async function drive(pad, alarmsOf) {
  ...
  // The walk takes minutes; an alarm that tripped since the start would stop
  // the log again, so check once more right before the press.
  const alarms = await alarmsOf();
  if (!alarms) return { error: "The alarm state could not be re-read before RESTART LOGS" };
  if (alarms.active.length) return { error: `${alarms.active.map((a) => a.name).join(", ")} became active during the re-arm` };
  await pad.key(KEY.ENTER);
...
async function rearmLogger({ source, pad, readAlarms: alarmsOf = readAlarms, post }) {
  ...
  try {
    ...
    let result;
    try { result = await drive(keypad, alarmsOf); } catch (e) { result = { error: `Keypad request failed (${e instanceof Error ? e.message : e})` }; }
    let home = false;
...
module.exports = { rearmLogger, MANUAL };
