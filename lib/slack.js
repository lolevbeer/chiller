...
// One env name per knob: the margin/drop thresholds live in lib/boost.js and
// are shared with the manual-fallback nudge below. (boost.js requires this
// module lazily at post time, so the load-time edge stays one-directional.)
const { t10, TRIP_F, BOOST_MARGIN_F, BOOST_URGENT_F, BOOST_DROP_F } = require("./boost");
const dateTime = require("./datetime");
const { logNewest } = require("./logcache");
const { MANUAL } = require("./pgd");
...
const LOG_STALE = Number(process.env.SLACK_LOG_STALE_MIN || 15); // min without a new datalogger row (tail polls every 5)
 ...
 * logNewest is the newest datalogger row (epoch ms); null while the log cache
 * is still backfilling, so a stale-looking log is never judged too early.
 * @typedef {{t: number,
 *            regs: Record<number, number> | null,
 *            web: Record<string, number> | null,
 *            alarms: {active: Array<{name: string, since: string}>} | null,
 *            logNewest?: number | null}} State
 */

const F = (/** @type {number} */ n) => `${n.toFixed(1)}°F`;
// Supply minus setpoint in integer tenths, the same way lib/boost.js decide()
// computes it, so the nudge and the automation agree on an exactly-13.0°F margin.
const over10 = (/** @type {State} */ s) => t10(scale(s.regs[68])) - t10(scale(s.regs[70]));
...
// Shared body of the shutdown-trip nudge (dwelled and urgent tiers below):
// what the firmware will do, and the exact setpoint to raise to and restore.
const boostNudge = (/** @type {State} */ s) => [
  ...glycolCtx(s),
  `The controller shuts the unit down when supply exceeds setpoint + ${TRIP_F}°F (hardcoded). Temporarily RAISE the setpoint to ${F(scale(s.regs[68]) - BOOST_DROP_F)} — ${BOOST_DROP_F}°F below the current ${F(scale(s.regs[68]))} supply — to restore margin while the compressors catch up. Restore the normal setpoint (${F(scale(s.regs[70]))}) once the loop is back down near it.`,
];

/** level is the plain-text severity label ("Critical" | "Warning") that
 * prefixes the alert title.
 * @typedef {{key: string, level: string, title: string,
 *             source?: keyof State, dwellMs?: number, samples?: number,
...
/** @type {Condition[]} */
const CONDITIONS = [
  { key: "offline", level: "Warning", title: "Chiller unreachable", samples: 2,
    test: (/** @type {State} */ s) => !s.regs,
    detail: () => ["Modbus reads are failing"] },

  // The onboard datalogger halts the instant any alarm fires and never re-arms
  // itself (lib/logcache.js). While an alarm stands, that alarm's own alert
  // covers it and a re-arm would only stop again, so this waits for a clean
  // controller. It clears only on proof (fresh rows), never on a missing read.
  { key: "logger-stopped", level: "Warning", title: "Datalogger stopped", source: "logNewest", samples: 2,
    test: (/** @type {State} */ s) => !!s.regs && !!s.alarms && !s.alarms.active.length &&
      s.t - s.logNewest > minute(LOG_STALE),
    clear: (/** @type {State} */ s) => s.t - s.logNewest <= minute(LOG_STALE),
    detail: (/** @type {State} */ s) => [
      `No new log rows for ${elapsed(s.t - s.logNewest)}; history charts are frozen until it is re-armed`,
      `Use the dashboard's Re-arm logger button or \`/chiller rearm\`. Or: ${MANUAL}`,
    ] },

  ...["A", "B"].map((c) => /** @type {Condition} */ ({
    key: `leak:${c}`, level: "Critical", title: `Propane detected — sensor ${c}`, source: "web", dwellMs: 0,
  ...
  ...["A", "B"].map((c) => /** @type {Condition} */ ({
    key: `leak-sensor:${c}`, level: "Warning", title: `Propane sensor ${c} stopped reporting`, source: "web", samples: 2,
    test: (/** @type {State} */ s) => !!s.web && !(`LEL ${c} %` in s.web),
    detail: () => ["Leak detection is unavailable for this circuit until the sensor reports again"],
  })),

  { key: "pstat:HP", level: "Critical", title: "High-pressure switch tripped", source: "web", dwellMs: 0,
    test: (/** @type {State} */ s) => s.web?.["HP pressostat trip"] === 1,
    detail: () => ["Mechanical safety device; the circuit remains locked out until it is reset"] },
  { key: "pstat:LP", level: "Critical", title: "Low-pressure switch tripped", source: "web", dwellMs: 0,
  ...
  { key: "glycol-crit", level: "Critical", title: "Critical glycol supply temperature", source: "regs", dwellMs: minute(HIGH_DWELL),
    test: (/** @type {State} */ s) => !!s.regs && scale(s.regs[68]) > CRIT_F,
    clear: (/** @type {State} */ s) => !!s.regs && scale(s.regs[68]) < CRIT_F - HYST_F,
    detail: glycolCtx },
  { key: "glycol-high", level: "Warning", title: "High glycol supply temperature", source: "regs", dwellMs: minute(HIGH_DWELL),
  ...
  // The manual fallback for the setpoint+TRIP_F firmware shutdown: the c.pCO
  // alarms and SHUTS THE UNIT OFF when supply exceeds setpoint + 18°F, and the
  // dodge is to temporarily RAISE the setpoint (supply − BOOST_DROP_F keeps the
  // compressors pulling while restoring the margin), then put the normal
  // setpoint back once the loop recovers. When SETPOINT_WRITE=1 lib/boost.js
  // performs and announces those writes itself, so this nudge stands down —
  // the env var is read per-evaluation so tests can flip it. Same dwell +
  // hysteresis as the bands above so it doesn't flap on the threshold.
  { key: "glycol-boost", level: "Warning", title: `Glycol supply within ${TRIP_F - BOOST_MARGIN_F}°F of the shutdown trip`, source: "regs", dwellMs: minute(HIGH_DWELL),
    test: (/** @type {State} */ s) => process.env.SETPOINT_WRITE !== "1" &&
      !!s.regs && over10(s) > BOOST_MARGIN_F * 10,
    clear: (/** @type {State} */ s) => process.env.SETPOINT_WRITE === "1" ||
      (!!s.regs && over10(s) < (BOOST_MARGIN_F - HYST_F) * 10),
    detail: boostNudge },

  // Urgent tier of the same nudge: past BOOST_URGENT_F the loop is climbing too
  // fast for the 5-minute dwell — the human would get the nudge after the
  // shutdown (the boost module skips its dwell past this threshold for exactly
  // this reason), so this fires on the first sample. Its own key, so a fast
  // climb posts the escalation even while the dwelled nudge is still counting.
  { key: "glycol-boost-urgent", level: "Critical", title: `Glycol supply within ${TRIP_F - BOOST_URGENT_F}°F of the shutdown trip`, source: "regs", dwellMs: 0,
    test: (/** @type {State} */ s) => process.env.SETPOINT_WRITE !== "1" &&
      !!s.regs && over10(s) > BOOST_URGENT_F * 10,
    clear: (/** @type {State} */ s) => process.env.SETPOINT_WRITE === "1" ||
      (!!s.regs && over10(s) < (BOOST_URGENT_F - HYST_F) * 10),
    detail: boostNudge },

  { key: "glycol-freeze", level: "Critical", title: "Glycol supply below the freeze floor", source: "regs", dwellMs: minute(5),
  ...
  ...[["A", "Chiller pump on"], ["B", "Process pump on"]].map(([c, pump]) => /** @type {Condition} */ ({
    key: `no-flow:${c}`, level: "Critical", title: `No glycol flow, circuit ${c}`, source: "web", dwellMs: minute(2),
  ...
  { key: "not-cooling", level: "Critical", title: "Compressors running but not cooling", source: "regs", dwellMs: 0,
  ...
  { key: "runtime-imbalance", level: "Warning", title: "Circuit runtime imbalance", source: "regs", dwellMs: 0,
...
function step(prev, s) {
  ...
  const dyn = (s.alarms?.active ?? []).map((a) => ({
    key: `alarm:${a.name}`, level: "Critical", title: a.name, source: "alarms", dwellMs: 0,
  ...
  for (const c of [...CONDITIONS, ...dyn]) {
    ...
    const sourceMissing = !!c.source && s[c.source] == null; // source names a State field
    if (sourceMissing) {
  ...
  if (!s.alarms) {
    for (const [k, v] of prev.active) if (k.startsWith("alarm:")) active.set(k, v);
  } else {
    // A cleared alarm is never visited by the loop above, so drop its dwell
    // counter here — otherwise a re-fire inherits the old incident's start time.
  ...
  for (const [k, v] of prev.active) {
    ...
    posts.push(`*Resolved: ${v.title}*\nIncident lasted ${elapsed(s.t - v.since)}` +
      (s.regs ? `\n${glycolCtx(s)[0]}` : ""));
  }
  return { active, counts, hist, posts };
}

// Side-bar color per severity label. Every message is written as
// "*Label: title*\n…", so the color is read from that prefix at send time.
const LABEL_COLORS = { Critical: "#d00000", Warning: "#e8a200", Resolved: "#2eb67d", Notice: "#1d9bd1" };

/** Move a labelled {text} message into a colored attachment. `fallback` keeps
 * the text in push notifications, which don't render attachment bodies.
 * Unlabelled bodies (and ones that already have attachments) pass through.
 * @param {any} body */
const colorize = (body) => {
  const color = LABEL_COLORS[/^\*(\w+):/.exec(body?.text ?? "")?.[1]];
  return color ? { attachments: [{ color, text: body.text, fallback: body.text }] } : body;
};

/**
 * @param {object} body Slack webhook body; a labelled {text} is sent colored (see colorize)
...
async function post(body, send = fetch, log = console.error) {
  try {
    const res = await send(SLACK_URL, {
      ...
      body: JSON.stringify(colorize(body)),
    });
...
async function slackTick() {
  ...
  const next = step(cond, { t: Date.now(), regs, web, alarms, logNewest: logNewest() });
  cond = next;
