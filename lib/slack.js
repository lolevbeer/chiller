...
const RES_LVL = Number(process.env.SLACK_RES_LVL || 20); // reservoir level below this = low
...
const CONDITIONS = [
  ...
  // ponytail: ResLvl's units were never correlated against the controller (the
  // level sensor is web-only) — hence the env knob and the conservative default.
  // The float switch itself raises Al_LowlvlSensor, which the alarm conditions
  // already carry, so this is the early warning, not the safety. Recalibrate
  // SLACK_RES_LVL once a real low-level event has been seen in the channel.
  { key: "reservoir-low", level: "⚠️", title: "Reservoir level low", source: "web", dwellMs: minute(5),
    test: (/** @type {State} */ s) => s.web?.["Reservoir level"] < RES_LVL,
    clear: (/** @type {State} */ s) => s.web?.["Reservoir level"] > RES_LVL * 1.1,
    detail: (/** @type {State} */ s) => [`Level reads ${s.web["Reservoir level"]} (warns below ${RES_LVL})`] },
...
function step(prev, s) {
  ...
  const dyn = (s.alarms?.active ?? []).map((a) => ({
    ...
    test: () => true, clear: undefined, detail: () => [`Controller alarm, standing since ${a.since}`],
  }));
  ...
  for (const [k, v] of prev.active) {
    ...
    posts.push(`✅ *${v.title} recovered*\nIncident lasted ${mins(s.t - v.since)}` +
      (s.regs ? `\n${glycolCtx(s)[0]}` : ""));
  }
...
const summary = (regs) => ({
  attachments: [{ color: "#cccccc", text: [
    `*Chiller daily summary* (${stats.since.toDateString() === new Date().toDateString()
      ? `since ${hhmm(stats.since)}` : "past 24 h"})`,
...
module.exports = { slackPayload, step, CONDITIONS, post, flushPosts, initialDailyKey, startSlack };
