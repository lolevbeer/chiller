...
const CONDITIONS = [
  ...
  { key: "logger-stopped", level: "Warning", title: "Datalogger stopped", source: "log", samples: 2,
    ...
    detail: (/** @type {State} */ s) => [
      ...
      "Re-arm it with the dashboard's Re-arm logger button or `/chiller rearm`, or on the controller keypad: hold Alarm + Enter about 3 s for the system menu, then LOGGER, then RESTART LOGS",
    ] },
...
function step(prev, s) {
  ...
  for (const c of [...CONDITIONS, ...dyn]) {
    ...
    const sourceMissing = c.source === "regs" ? s.regs === null
      ...
      : c.source === "alarms" ? s.alarms === null
      : c.source === "log" ? s.logNewest == null : false;
    if (sourceMissing) {
