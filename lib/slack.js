...
const { TRIP_F, BOOST_MARGIN_F, BOOST_URGENT_F, BOOST_DROP_F } = require("./boost");
...
const CONDITIONS = [
  ...
  { key: "glycol-boost", level: "Warning", title: `Glycol supply within ${TRIP_F - BOOST_MARGIN_F}°F of the shutdown trip`, source: "regs", dwellMs: minute(HIGH_DWELL),
    // Margin compared in integer tenths (like lib/boost.js decide): float
    // subtraction of the scaled x10 values makes an exactly-13.0°F margin trip
    // for some setpoint values and not others.
    test: (/** @type {State} */ s) => process.env.SETPOINT_WRITE !== "1" &&
      !!s.regs && Math.round((scale(s.regs[68]) - scale(s.regs[70])) * 10) > BOOST_MARGIN_F * 10,
    clear: (/** @type {State} */ s) => process.env.SETPOINT_WRITE === "1" ||
      (!!s.regs && Math.round((scale(s.regs[68]) - scale(s.regs[70])) * 10) < (BOOST_MARGIN_F - HYST_F) * 10),
    detail: boostNudge },
  ...
  { key: "glycol-boost-urgent", level: "Critical", title: `Glycol supply within ${TRIP_F - BOOST_URGENT_F}°F of the shutdown trip`, source: "regs", dwellMs: 0,
    test: (/** @type {State} */ s) => process.env.SETPOINT_WRITE !== "1" &&
      !!s.regs && Math.round((scale(s.regs[68]) - scale(s.regs[70])) * 10) > BOOST_URGENT_F * 10,
    clear: (/** @type {State} */ s) => process.env.SETPOINT_WRITE === "1" ||
      (!!s.regs && Math.round((scale(s.regs[68]) - scale(s.regs[70])) * 10) < (BOOST_URGENT_F - HYST_F) * 10),
    detail: boostNudge },
