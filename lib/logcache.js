...
/** Newest cached row (epoch ms), or null while the backfill runs or the cache
 * is empty — "unknown", which lib/slack.js never mistakes for a stopped log. */
const logNewest = () => (loading || !logCache.length ? null : logCache[logCache.length - 1][0]);

module.exports = { TSTAMP, readLog, logInsert, logSlice, logLoop, logLoading, logNewest };
