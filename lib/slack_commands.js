...
/** @param {string} value */
const alarmMoment = (value) => esc(dateTime.moment(value));

/** @param {string} startValue @param {string} stopValue */
const readableRange = (startValue, stopValue) => esc(dateTime.range(startValue, stopValue));

/** @param {{at: string, cleared: string | null}} alarm */
function alarmHistoryTime(alarm) {
  if (!alarm.cleared) return `${alarmMoment(alarm.at)} · clear time unavailable`;
  const elapsed = dateTime.elapsed(alarm.at, alarm.cleared);
  const duration = elapsed === 0 ? "cleared immediately"
    : elapsed === null ? null : `lasted ${dateTime.duration(elapsed)}`;
  return `${readableRange(alarm.at, alarm.cleared)}${duration ? ` · ${duration}` : ""}`;
}
...
const HELP = [
  ...
  "`/chiller rearm` — restart the controller's datalogger after an alarm stopped it (presses keys on the controller)",
 ...
 *                  logLoading: typeof logLoading, now: () => number,
 *                  rearmLogger: typeof rearmLogger}>} deps */
async function commandResponse(input, deps = {}) {
  const io = { read, readWeb, readAlarms, logSlice, logLoading, rearmLogger, now: Date.now, ...deps };
  ...
  if (command === "status") {
    ...
    // lib/pgd.js refuses while an alarm stands and posts its own audit message.
    const r = await io.rearmLogger({ source: "Slack" });
    text = `*Datalogger ${r.ok ? "re-armed" : "not re-armed"}*\n${r.message}`;
  } else {
    text = HELP;
  }
