// Automatic setpoint boost — one of the two SETPOINT_WRITE-gated write paths
// in an otherwise read-only app (the other is the dashboard's manual
// POST /api/setpoint in lib/routes.js).
//
...
// so when supply climbs to within 5°F of that trip (margin > BOOST_MARGIN_F for
// BOOST_DWELL_MIN samples, or past BOOST_URGENT_F on a single sample when the
// loop is rising too fast to sit through the dwell)
...
const BOOST_URGENT_F = Number(process.env.BOOST_URGENT_F || 15); // °F over setpoint that raises on the FIRST sample, skipping the dwell
...
const TH = { MARGIN_F: BOOST_MARGIN_F, URGENT_F: BOOST_URGENT_F, DROP_F: BOOST_DROP_F, RESTORE_F: BOOST_RESTORE_F,
 ...
 *
 * The ordering MARGIN_F < URGENT_F < TRIP_F is checked for the same reason: an
 * URGENT_F at or below MARGIN_F turns every ordinary excursion into a no-dwell
 * write (the bypass gates on it), and one at or past TRIP_F can never fire
 * before the firmware shutdown — the exact race the bypass exists to close. A
 * retune that breaks the ordering must be a loud startup refusal, not a silent
 * behavior change.
...
const validBoostThresholds = (th = TH) =>
  ...
  [th.MARGIN_F, th.URGENT_F, th.DROP_F, th.RESTORE_F, th.CEIL_F].every(Number.isFinite) &&
  th.MARGIN_F < th.URGENT_F && th.URGENT_F < TRIP_F;
 ...
 * - Urgent bypass: margin > URGENT_F skips the dwell — the loop is climbing
 *   too fast to wait it out — raising on the first sample of a fresh incident
 *   and after one over-margin sample of spacing between raises mid-incident.
 *   The bypass applies ONLY when a useful raise is available (write budget
 *   left, target ≥ 1°F above the setpoint): ceiling-bound or write-capped
 *   incidents wait out the full dwell, so ceiling-alert never fires early or
 *   more than once per dwell.
...
function decide(prev, s, th = TH) {
  ...
  // This guard is also why the dashboard's manual setpoint control (POST
  // /api/setpoint) needs no coordination with the boost: a manual write
  // mid-incident reads as tampering here and the automation stands down on
  // the next tick.
  ...
  const over = t10(supplyF) - t10(setpointF);
  const trigCount = over > t10(th.MARGIN_F) ? prev.trigCount + 1 : 0;
  const state = { ...prev, trigCount, restoreCount };
  const targetF = r1(Math.min(supplyF - th.DROP_F, th.CEIL_F));
  // Shared by the urgent gate and the two alert branches below — deriving both
  // from the same values means the gate can never drift out of sync with the
  // branches it screens for (which would resurrect per-poll alert spam).
  const writesLeft = prev.writes < th.MAX_WRITES;
  const usefulRaise = t10(targetF) - t10(setpointF) >= 10; // ≥ 1°F, in tenths
  // The dwell filters transient spikes, but a loop climbing ~1°F/min reaches the
  // trip before DWELL polls elapse — that race is what let the firmware alarm
  // fire before the first raise. Past URGENT_F the spike question is moot: raise
  // on the first sample of a fresh incident. Mid-incident (writes > 0) one
  // over-margin sample of spacing is required between raises: a flapping sensor
  // could otherwise burn a write on every consecutive poll and drain MAX_WRITES
  // in minutes, while a real climb loses only one poll. Gated on a raise
  // actually being available so a ceiling-bound or write-capped incident still
  // waits out the dwell and can't alert-spam. The bypass is a shortened dwell,
  // so it carries the same `DWELL >= 1` sanity check as the main gate — a
  // malformed threshold set must disable the automation, not route around it —
  // and a NaN URGENT_F satisfies no comparison, keeping it shut.
  const urgent = th.DWELL >= 1 && over > t10(th.URGENT_F) && writesLeft && usefulRaise &&
    trigCount >= (prev.writes > 0 ? 2 : 1);
  ...
  if (!(trigCount >= th.DWELL || urgent)) return { state, action: null };
...
async function boostCycle() {
  ...
  if (action.type === "ceiling-alert") { await notify(`*Critical: Chiller shutdown may be imminent; setpoint boost cannot prevent it*\n${action.reason}`); return; }
  if (action.type === "abort") { await notify(`*Warning: Setpoint boost aborted*\n${action.reason}`); return; }
  ...
  if (res.ok) {
    ...
    await notify(action.type === "raise"
      ? `*Notice: Chiller setpoint raised from ${F(s.setpointF)} to ${F(action.targetF)}*\n${action.reason}. The setpoint was raised to maintain ${BOOST_DROP_F}°F of margin while the compressors bring the loop down. The original setpoint (${F(state.originalF)}) will be restored automatically once supply is within ${BOOST_RESTORE_F}°F of it.`
      : `*Resolved: Chiller setpoint restored to ${F(action.targetF)}*\n${action.reason}. The loop temperature has recovered and the normal setpoint is in effect.`);
    ...
    state = { ...IDLE };
    await saveState();
    await notify(`*Critical: Setpoint boost disabled; write went to the wrong register*\nWrote ${F(action.targetF)}, but the setpoint reads back ${F(res.readback)}. SETPOINT_WREG appears to be incorrect. No further writes will be made until the register is re-verified with probe_setpoint.js and boost_state.json is deleted.${prev.originalF != null ? ` Please verify the controller; the intended setpoint was ${F(prev.originalF)}.` : ""}`);
  } else if (res.wrote) {
    ...
    await saveState();
    await notify(`*Warning: Setpoint ${action.type} sent but not verified (${res.error || "no response"})*\nWrote ${F(action.targetF)}, but the readback did not complete. The next poll will confirm whether the write took effect. ${action.reason}.`);
  } else {
    ...
    if (failedWrites > BOOST_MAX_WRITES) {
      ...
      failedWrites = 0;
      await saveState();
      await notify(`*Critical: Setpoint boost stopped after ${BOOST_MAX_WRITES} failed write attempts*\nThe chiller is not accepting Modbus writes. Please set the setpoint manually${prev.originalF != null ? ` (normal setpoint: ${F(prev.originalF)})` : ""}; the controller shuts down at setpoint + ${TRIP_F}°F.`);
    } else {
      state = prev;
      if (failedWrites === 1) await notify(`*Warning: Setpoint write failed (${res.error || "no response"}); retrying each poll*\nAttempted to ${action.type} the setpoint to ${F(action.targetF)}. ${action.reason}. Further failures will be reported only if the boost stops retrying.`);
    }
  }
}
...
/**
 * Live snapshot for the dashboard (lib/routes.js folds it into /api/all under
 * `boost`): enough to render the "boost active" line and the latched warning
 * without exposing the module's internals. `enabled` reads the env per call so
 * it always reflects the same gate every write path uses; maxWrites rides
 * along so the page can say "write N of M" without hardcoding BOOST_MAX_WRITES.
 * @returns {{enabled: boolean, phase: BoostState["phase"], originalF: number | null,
 *            lastWrittenF: number | null, writes: number, maxWrites: number, latched: boolean}}
 */
const boostStatus = () => ({ enabled: process.env.SETPOINT_WRITE === "1",
  phase: state.phase, originalF: state.originalF, lastWrittenF: state.lastWrittenF,
  writes: state.writes, maxWrites: BOOST_MAX_WRITES, latched });

module.exports = { t10, decide, reviveState, validBoostThresholds, IDLE, TH, TRIP_F, BOOST_MARGIN_F, BOOST_URGENT_F, BOOST_DROP_F, startBoost, boostStatus };
