 ...
 *   consecutive samples — or > URGENT_F on a single sample, when the loop is
 *   climbing too fast to wait out the dwell — raises to
 *   min(supply − DROP_F, CEIL_F). When that
...
function decide(prev, s, th = TH) {
  ...
  // on this sample. Gated on a raise actually being available so a ceiling-bound
  // or write-capped incident still waits out the dwell and can't alert-spam.
  const canRaise = prev.writes < th.MAX_WRITES && t10(targetF) - t10(setpointF) >= 10;
  // Inverted (not `trigCount < DWELL`) so a NaN dwell fails safe: NaN satisfies
  // no comparison, and falling through here would raise on the FIRST sample. The
  // bypass is a dwell of one, so it carries the same `DWELL >= 1` sanity check —
  // a malformed threshold set must disable the automation, not route around it.
  const urgent = th.DWELL >= 1 && over > t10(th.URGENT_F) && canRaise;
...
module.exports = { decide, reviveState, validBoostThresholds, IDLE, TH, TRIP_F, BOOST_MARGIN_F, BOOST_DROP_F, startBoost, boostStatus };
